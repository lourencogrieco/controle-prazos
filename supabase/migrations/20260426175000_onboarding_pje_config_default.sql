-- Empresas novas devem nascer com uma linha de pje_config.
-- Sem isso, o update de configuracoes nao encontrava registro para empresas recém-criadas.
...
insert into public.pje_config (empresa_id, nomes, ativo)
select e.id::text, '{}', true
from public.empresas e
where not exists (
  select 1
  from public.pje_config pc
  where pc.empresa_id = e.id::text
);
